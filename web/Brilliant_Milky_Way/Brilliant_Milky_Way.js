function brilliantMilkyWay(CanvasEle) {
    // 定义星星的颜色
    ...
    function movePointer(x, y) {
        ...
        if (typeof pointerX === 'number' && typeof pointerY === 'number') {
            let ox = x - pointerX;
            let oy = y - pointerY;
            velocity.tx = velocity.tx + (ox / 15) * scale * (touchInput ? 1 : -1);
            velocity.ty = velocity.ty + (oy / 15) * scale * (touchInput ? 1 : -1);
        }
        // 更新鼠标指针的位置
        pointerX = x;
        pointerY = y;
...
export default brilliantMilkyWay;